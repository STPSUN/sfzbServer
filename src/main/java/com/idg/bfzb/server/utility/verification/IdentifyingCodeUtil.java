package com.idg.bfzb.server.utility.verification;
...
/**
 * 生成图片验证码
 * Created by zlgant on 2015/12/17.
 */
public class IdentifyingCodeUtil {
    ...
    public String createImage(String path){
        ...
        this.drawRandomLines(g, 160);
        String code = this.drawRandomString(4, g) ;
        try{
            ImageOutputStream imageIo = ImageIO.createImageOutputStream(file);
            ImageIO.write(image, "jpg", imageIo);
        }catch (Exception e) {
            e.printStackTrace();
        }
        return code;
