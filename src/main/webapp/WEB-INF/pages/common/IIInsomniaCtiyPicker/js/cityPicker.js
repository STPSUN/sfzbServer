...
IIInsomniaCityPicker.prototype = {
    ...
    buildProvinceTpl: function(){
        ...
        for(var i = 0, len = province.length; i < len; i++){
        	var provinceShortName = province[i]['name'];
        	if(provinceShortName.length > 3){
        		provinceShortName = provinceShortName.substr(0,2) + "...";
        	}
            province_html += '<li class="IIInsomnia-province" data-id="' + province[i]['id'] + '" data-name="' + province[i]['name'] + '" data-code="' + province[i]['code'] + '"><ul class="IIInsomnia-city-wrap"></ul><div class="IIInsomnia-province-name" title="'+province[i]['name']+'" >' + provinceShortName + '</div></li>';
        }
    ...
    buildCityTpl: function(cur_province){
        ...
        for(var j = 0, clen = city.length; j < clen; j++){
            city_html += '<li class="IIInsomnia-city" data-id="' + city[j]['id'] + '" data-name="' + city[j]['name'] + '" data-code="' + city[j]['code'] + '" title="' + city[j]['name'] + '">' + city[j]['name'] + '</li>';
        }
    ...
    cityEvent: function(){
        ...
        that.province_wrap.on('click', '.IIInsomnia-city', function(event) {
            ...
            var cid = _this.data('id');
            var cname = _this.data('name');
            var ccode = _this.data('code');
            console.log("选择城市code:"+ccode);

            that.settings.target.val(cname);

            if(that.settings.hideCityInput){
                if(that.settings.valType == 'k-v'){
                    that.settings.hideCityInput.val(cid + '-' + cname + '-' + ccode);
                }else{
                    that.settings.hideCityInput.val(cid);
                }
            }
            ...
            if(that.settings.hideProvinceInput){
                ...
                var pcode = pele.data('code');
                if(that.settings.valType == 'k-v'){
                    that.settings.hideProvinceInput.val(pid + '-' + pname + '-' + pcode);
                }else{
                    that.settings.hideProvinceInput.val(pid);
                }
            }
